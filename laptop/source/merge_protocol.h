...
void writeByte(uint8_t b, uint16_t *checksum, HANDLE com) {
    if(checksum != NULL) (*checksum) += b;

    if(b == 0xFE) {
        //0xFE -> 0xEE 0x00
        uint8_t bs[] = { 0xEE, 0x00 };
        DWORD number_of_bytes_written;
        WriteFile(com, bs, 2, &number_of_bytes_written, NULL);
    } else if(b == 0xEE) {
        //0xEE -> 0xEE 0x01
        uint8_t bs[] = { 0xEE, 0x01 };
        DWORD number_of_bytes_written;
        WriteFile(com, bs, 2, &number_of_bytes_written, NULL);
    } else {
        DWORD number_of_bytes_written;
        WriteFile(com, &b, 1, &number_of_bytes_written, NULL);
    }
}

void sendPacket(uint8_t *payload, uint16_t len, uint8_t type, HANDLE com) {
    if(com == INVALID_HANDLE_VALUE) return;
    ...
    uint16_t checksum = 0xFE;
    {
        ...
        DWORD number_of_bytes_written;
        WriteFile(com, &start_byte, 1, &number_of_bytes_written, NULL);
    }

    writeByte((uint8_t)((len >> 0) & 0xFF), &checksum, com); //PayloadLen LSB
    writeByte((uint8_t)((len >> 8) & 0xFF), &checksum, com); //PayloadLen MSB
    writeByte(type, &checksum, com); //PacketType

    //Payload    
    for(int j = 0; j < len; j++) writeByte(payload[j], &checksum, com);

    writeByte((uint8_t)((checksum >> 0) & 0xFF), NULL, com); //Checksum LSB
    writeByte((uint8_t)((checksum >> 8) & 0xFF), NULL, com); //Checksum MSB
}